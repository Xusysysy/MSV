...
| — Search 步 | 胶囊搜索框 + **最近搜索横向卡片（LazyRow L514，紧凑 8dp 圆角 / 4dp 内边距 / 最大 104dp，点击直接搜索 / 清空）** + 说明文案；**搜索步高度随内容自适应（Modifier.heightIn max 0.92 屏高）→ 卡片与文案完整可见、无需滚动**（滚动仅作超小屏/横屏兜底） | L464-L540 |
